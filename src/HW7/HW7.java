...
public class HW7 {
    ...
    public static boolean hasDuplicate(String[] aos) {
        for (int i = 0; i < aos.length - 1; i++) {
            for (int s = i + 1; s < aos.length; s++) {
                if (aos[i].equals(aos[s])) {
                    return true;
                }
            }
    ...
    public static boolean isAnagram(String s1, String s2) {
        char[] charOf1 = make_array(s1);
        char[] charOf2 = make_array(s2);
        Arrays.sort(charOf1);
        Arrays.sort(charOf2);
        if (Arrays.equals(charOf1, charOf2)) {
            return true;
        }
        return false;
    }
    ...
    public static int numDifferent(int[] nums) {
        ...
        return total; // shut up error message
    }
